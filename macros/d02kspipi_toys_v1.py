...
# Acceptance factory (gera d, p, t0, a_base, etc.)
...
parser.add_argument("--output", type=str, default="d02kspipi_toy", help="Output file stem (without extension)")
parser.add_argument("--dryrun", action="store_true", help="Parse & exit without running")
# switches da aceitação
parser.add_argument("--use_acceptance", action="store_true", help="Apply Dalitz×time acceptance")
parser.add_argument("--rho", type=float, default=0.8, help="ACC correlation strength (a(s)=a_base*(1+rho*z))")
parser.add_argument("--alpha", type=float, default=1.4, help="Slope for Dalitz efficiency eps_s ~ z^alpha")
...
def mixing_model(x):
    ...
    def _model(x_mix_par, y_mix_par, qoverp_re, qoverp_im):
        ...
        return atfm.mixing_density(
            ampl_dz, ampl_dzb, atfi.complex(qoverp_re, qoverp_im), tep, tem, tei
        )
    return _model
...
# -------------------- acceptance hook --------------------
def attach_acceptance_if_requested():
    if not args.use_acceptance:
        return
    # constrói modelo, anexa ao phsp e deixa pronto para uso em acceptance_weight
    acc_model = make_time_model(rho=args.rho)
    acc_model.dp = phsp
    phsp.set_acceptance_model(
        acc_model, rho=args.rho, alpha=args.alpha, eps_min=args.eps_min
    )
...
def density_with_optional_acc(x):
    ...
    if not args.use_acceptance:
        return dens
    s = c_phsp.data1(x)                       # [N,2] (m2ab,m2bc)
    t = c_phsp.phsp2.t(c_phsp.data2(x))       # [N,1] time
    w = phsp.acceptance_weight(s, t)          # ε(s)*f(t|s)
    return dens * w
...
def main():
    ...
    attach_acceptance_if_requested()
